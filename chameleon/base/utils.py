...
from .components import build_component, build_component_cls
...
def replace_module(
    ...
    target = build_component_cls(target) if isinstance(target, str) else target
    dst_module = build_component(**dst_module) if isinstance(dst_module, dict) else dst_module
